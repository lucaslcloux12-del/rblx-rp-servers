...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsubscribeData = onSnapshot(doc(db, "settings", "servers"), (docSnap) => {
      if (docSnap.exists()) {
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-white to-blue-50">
        <div className="max-w-2xl w-full px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Servidor 1 */}
            <a href={serverData.status1 ? serverData.link1 : "#"} target={serverData.status1 ? "_blank" : undefined}
               className={`block bg-white rounded-3xl p-8 shadow-xl border transition-all ${
                 serverData.status1 ? 'hover:shadow-2xl hover:-translate-y-1 border-green-200 cursor-pointer' : 'pointer-events-none opacity-60 cursor-not-allowed border-gray-300'
               }`}>
              <div className="flex items-center gap-4">
                <span className="text-5xl">🟢</span>
                <div className="flex-1">
                  <div className="text-2xl font-semibold">Servidor 1</div>
                  <div className={`text-sm ${serverData.status1 ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {/* Servidor 2 */}
            <a href={serverData.status2 ? serverData.link2 : "#"} target={serverData.status2 ? "_blank" : undefined}
               className={`block bg-white rounded-3xl p-8 shadow-xl border transition-all ${
                 serverData.status2 ? 'hover:shadow-2xl hover:-translate-y-1 border-yellow-200 cursor-pointer' : 'pointer-events-none opacity-60 cursor-not-allowed border-gray-300'
               }`}>
              <div className="flex items-center gap-4">
                <span className="text-5xl">🟡</span>
                <div className="flex-1">
                  <div className="text-2xl font-semibold">Servidor 2</div>
                  <div className={`text-sm ${serverData.status2 ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {/* Servidor 3 */}
            <a href={serverData.status3 ? serverData.link3 : "#"} target={serverData.status3 ? "_blank" : undefined}
               className={`block bg-white rounded-3xl p-8 shadow-xl border transition-all ${
                 serverData.status3 ? 'hover:shadow-2xl hover:-translate-y-1 border-red-200 cursor-pointer' : 'pointer-events-none opacity-60 cursor-not-allowed border-gray-300'
               }`}>
              <div className="flex items-center gap-4">
                <span className="text-5xl">🔴</span>
                <div className="flex-1">
                  <div className="text-2xl font-semibold">Servidor 3</div>
                  <div className={`text-sm ${serverData.status3 ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {/* Servidor 4 */}
            <a href={serverData.status4 ? serverData.link4 : "#"} target={serverData.status4 ? "_blank" : undefined}
               className={`block bg-white rounded-3xl p-8 shadow-xl border transition-all ${
                 serverData.status4 ? 'hover:shadow-2xl hover:-translate-y-1 border-purple-200 cursor-pointer' : 'pointer-events-none opacity-60 cursor-not-allowed border-gray-300'
               }`}>
              <div className="flex items-center gap-4">
                <span className="text-5xl">🟣</span>
                <div className="flex-1">
                  <div className="text-2xl font-semibold">Servidor 4</div>
                  <div className={`text-sm ${serverData.status4 ? 'text-green-600' : 'text-gray-500'}`}>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-8">
            ...
            <div className="space-y-8 max-h-[70vh] overflow-y-auto pr-2">
              {[1,2,3,4].map(i => {
                const key = `link${i}` as keyof typeof editData;
                const statusKey = `status${i}` as keyof typeof editData;
                const emoji = ['🟢','🟡','🔴','🟣'][i-1];
                return (
                  <div key={i} className="border border-gray-200 rounded-3xl p-5">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xl">{emoji} Servidor {i}</span>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" checked={!!editData[statusKey]} onChange={(e) => setEditData({...editData, [statusKey]: e.target.checked})} className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-300 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500"></div>
                      </label>
                    </div>
                    <input type="text" value={editData[key] || ''} onChange={(e) => setEditData({...editData, [key]: e.target.value})} className="w-full border border-gray-300 rounded-2xl px-4 py-3 text-sm" placeholder={`Link do Servidor ${i}`} />
                  </div>
                );
              })}
            </div>
            ...
            <div className="flex gap-4 mt-10">
              ...
              <button onClick={saveChanges} disabled={isSaving} className="flex-1 py-4 rounded-3xl bg-blue-600 text-white font-medium hover:bg-blue-700 disabled:bg-gray-400">
